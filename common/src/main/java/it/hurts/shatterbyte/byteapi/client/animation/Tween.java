...
import it.hurts.shatterbyte.byteapi.util.AnimationUtils;
...
public class Tween {
    ...
    private void startTweeners() {
        if (tweeners.isEmpty()) {
            dead = true;
            ByteAPI.LOGGER.warn("Tween without commands, aborting.");
            return;
        }
    ...
    public void play() {
        if (dead) ByteAPI.LOGGER.warn("Can't play finished Tween, use stop() first to reset its state.");
        this.running = true;
    }
    ...
    protected boolean step() {
        ...
        if (!started) {
            if (tweeners.isEmpty()) {
                ByteAPI.LOGGER.warn("Tween started with no tweeners :(");
                return false;
            }
