...
public class ToggleWidget extends AbstractEntryWidget<Boolean> {
    public ToggleWidget(ShatterConfig config, FieldWidget parent, Boolean defaultValue, Supplier<Boolean> getter, Consumer<Boolean> setter) {
        super(config, parent, defaultValue, getter, setter, 0, 0, 14, 9);
    }
    ...
    protected void renderEntry(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        if (this.getValue()) {
            ...
            UIElements.SLIDER_THINGY.render(guiGraphics, RenderPipelines.GUI_TEXTURED, this.getX()+7, this.getY());
        } else {
            UIElements.TOGGLE_DISABLED.render(guiGraphics, RenderPipelines.GUI_TEXTURED, this.getX(), this.getY()+1);
            UIElements.SLIDER_THINGY.render(guiGraphics, RenderPipelines.GUI_TEXTURED, this.getX(), this.getY());
        }
    }
