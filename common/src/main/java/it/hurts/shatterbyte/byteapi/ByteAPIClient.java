...
import it.hurts.shatterbyte.byteapi.client.config.EntryWidgetFactory;
import it.hurts.shatterbyte.byteapi.client.config.EntryWidgetRegistry;
import it.hurts.shatterbyte.byteapi.client.config.widget.*;
import it.hurts.shatterbyte.byteapi.client.screen.TestGearScreen;
...
import it.hurts.shatterbyte.byteapi.module.config.dev.MyClientConfig;
...
import it.hurts.shatterbyte.byteapi.util.DeltaTimeTracker;
...
public final class ByteAPIClient {
    public static final ShatterConfig CONFIG = new MyClientConfig();

    public static void init() {
        ByteAPINetwork.registerS2CReceiver(TestScreenPacket.TYPE, TestScreenPacket.STREAM_CODEC, value -> {
            Minecraft.getInstance().setScreen(new TestGearScreen());
        });

        ByteAPINetwork.registerS2CReceiver(SyncServerConfigPacket.TYPE, SyncServerConfigPacket.STREAM_CODEC, value -> {
        ...
        if (ByteAPIServices.platform().isDevelopmentEnvironment()) {
            ConfigManager.register(ByteAPI.MOD_ID, CONFIG);
        }
    }
